...
public class Day14_1 {
	...
	public static void main(String[] args) {
		...
		// Math 클래스 : 수학 계산 관련 메소드 제공 
			...
			System.out.println(" 내림 : " + Math.floor(5.3) );
			System.out.println(" 내림 : " + Math.floor(-5.3) );
			...
			System.out.println(" 랜덤객체 : " + random.nextInt() );
			System.out.println(" 랜덤[0~1] : " + Math.random() );
			
			System.out.println(" 가까운 정수의 실수값 : " + Math.rint(5.5) );
			System.out.println(" 가까운 정수의 실수값 : " + Math.rint(-5.5) );
			
			System.out.println(" 반올림 : " + Math.round(5.5) );
			System.out.println(" 반올림 : " + Math.round(-5.5) );
			
			// 문제1 : 12.3456;  소수 둘째짜리에서 반올림 => 12.35
			double 실수4 = 12.3756;
				// 1. 자릿수 올리기 
				double temp =  실수4 * 10;
				// 2. 반올림 => 자릿수 내리기 
				실수4 = ( Math.round(temp) ) /10.0;
				
				System.out.println("반올림 후 : " + 실수4 );
			
			// 문제2 : 차량번호 랜덤으로 0000 ~ 9999 사이의 난수를 10개 생성 
					// 1. [차량번호] 난수 10개 배열에 저장 
					// 2. 차량번호 끝자리가 홀수 // 차량번호 끝자리 짝수 출력 
			
				// 1. 랜덤객체 선언 
				Random 랜덤 = new Random();
				//int[] 차량번호 = new int[10];
				String[] 차량번호 = new String[10];  // 차량번호 4자리 10개 저장하는 배열 
				String[] 홀수차량 = new String[10];	// 홀수차량 번호를 저장하는 배열
				String[] 짝수차량 = new String[10];	// 짝수차량 번호를 저장하는 배열 
				
				// 2. 난수 생성 10개 
				for( int i = 0 ; i<10 ; i++ ) {
					//차량번호[i]= 랜덤.nextInt(10000); // 0~9999 사이의 난수 생성 
					
					차량번호[i] = String.format( "%04d" , 랜덤.nextInt(10000) );
								// String.format( "형식문자" , 객체[데이터] )
									// * 형식문자 
										// %d : 정수 	%f : 실수 	%s : 문자열
										// 자릿수 
											// %숫자d : 데이터가 차지하는 자릿수 
				...
				for( String string : 홀수차량 ) { if(string !=null)System.out.println( string ); }
				// for 활용 
				// for( 자료형 임시객체 : 배열명 ) 
					// 배열내 인덱스 0번부터 끝까지 하나씩 임시객체에 대입 
				
				System.out.println("-------- 짝수차량 목록 ----------");
				for( String string  : 짝수차량 ) { if(string !=null)System.out.println( string ); }
